...
class Lab extends Component {
		...
		return (
			<div className="patient_profile_route">
				<div id="lab_container">
					...
					<div id="patient_lab_details_container">{this.render_columns()}</div>

					<button
						id="add_lab_details_btn"
						onClick={() => this.toggle_add_lab_panel()}
					>
	...
	render_lab_analysis_list() {
		return (
			<div id="lab_analysis_div_container">
				{this.state.lab_analysis_list.map((item, x) => (
					<div id="lab_analysis_div" key={x}>
						{item}
					</div>
				))}
			</div>
	...
	render_columns() {
		return (
			<div id="patient_lab_columns_container">
	...
	render_lab_analysis_div(lab, lab_analysis_item, key) {
		let filtered_tests = lab.tests.filter(test => {
			return test.name === lab_analysis_item;
		});

		return filtered_tests.length > 0 ? (
			<div key={key} id="lab_test_exists">
				{filtered_tests[0]["result"]}
			</div>
		) : (
			<div key={key} id="lab_test_not_exists" />
		);
	}
